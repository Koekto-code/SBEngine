...
static void loadCubemap(int& state);
...
int init(GLFWwindow*& wnd)
{
	...
	wnd = glfwCreateWindow(WinW, WinH, "SB engine 0.4.0 alpha", NULL, NULL);
	if (!wnd)
	{
		glfwTerminate();
		return -102;
...
void loadCubemap(int& state)
{
	...
	for (unsigned int i = 0; state && i < 6; ++i)
	{
...
void display(Camera& cam, sbObject& sb_obj)
{
	...
	// Display objects
	{
		...
		stdShader.setVec3("viewPos", cam.wPos);
