...
static char skybox = 3U; // sky texture: 3- display; 2- available, don't display; 0- not available

static uint8_t mouseState = 0U; // all buttons released
...
public:
	...
	void set_mass(float m/*, glm::vec3* ptr = nullptr, uint8_t q = 1U*/)
	{
		mass = m;
		/*if (ptr)
		{
	...
	void updatePhysics(double delta) // deltaTime for this
	{
		...
		if (!ph_exclength(glm::dvec3(0.0, 0.0, 0.0), coords, sunRadius + r))
		{
			...
			{
				glm::vec3 vert(surface[c*5U], surface[c*5U+1U], surface[c*5U+2U]); // get x, y, z
				vert += coords;
		...
		if (rotateVec != glm::vec3(0.f, 0.f, 0.f)) { // otherwise it gets NaN vector
			...
			for (uint16_t i = 0U; i < sizeof(ph_model) / 20U; i++)
			{
...
private:
	...
	//uint8_t mpts = 0U; // quantity of mass points
...
static void processInput(GLFWwindow* window)
{
	...
	for (uint8_t k = 0U; k < sph.size(); k++) {
		if (mouseState & 1U) // if LMB is pressed
		{
			if (!ph_exclengthf(cameraPos, sph[k].coords, 8.f))
				sph[k].speed += cameraForward * (float)deltaTime * 75.f;
		}
		else if (mouseState & 2U) // if RMB is pressed
		{
			if (!ph_exclengthf(cameraPos, sph[k].coords, 8.f))
